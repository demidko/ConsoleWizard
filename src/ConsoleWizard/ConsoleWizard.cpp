...
namespace
{
	...
	template<typename T> ConsoleWizard::Stream &Read(ConsoleWizard::Stream &s, T &v)
	{
		if (IsNewLine())
		{
			std::cout << s.Spaces();
	...
	template<typename T> ConsoleWizard::Stream &Print(ConsoleWizard::Stream &s, const T &v)
	{
		if (IsNewLine())
		{
			std::cout << s.Spaces();
...
namespace ConsoleWizard
{
	...
	Procedure::Procedure(const std::string_view & s) : Name(s)
	{
		io << "Процедура " << Name << " начата: " << tab;
	}
	
	Procedure::~Procedure()
	{
		io << untab << "Процедура " << Name << " завершена.";
	}
}